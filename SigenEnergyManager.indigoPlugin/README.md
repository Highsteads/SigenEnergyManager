...
| Version | Date | Notes |
|---------|------|-------|
| 5.21.1 | 23-May-2026 | **Site coordinates moved into the IndigoSecrets pattern; public defaults are now Big Ben.** Plugin now reads `LATITUDE` / `LONGITUDE` from `IndigoSecrets.py` first, PluginConfig (`siteLatitude` / `siteLongitude`) next, and only falls back to the built-in defaults if neither is set. Built-in defaults changed from the developer's home (Highsteads, 54.882 / -1.818) to Big Ben, London (51.5007, -0.1246) so a fresh install of the plugin doesn't leak the developer's location. PluginConfig label updated to explain the resolution order. Run-Self-Test secrets block now lists `LATITUDE` / `LONGITUDE` alongside the other keys. `IndigoSecrets_example.py` comment updated to point at SigenEnergyManager and the new fallback. |
...
# Site coordinates (v5.21.1 — optional; PluginConfig fallback + Big Ben default)
LATITUDE            = 51.5007
LONGITUDE           = -0.1246
...
| **Site latitude** *(v5.0, secrets-aware v5.21.1)* | Degrees N for Open-Meteo forecast. Read from `IndigoSecrets.py` (`LATITUDE`) first, this field next, default 51.5007 (Big Ben) last. |
| **Site longitude** *(v5.0, secrets-aware v5.21.1)* | Degrees E, negative for W. Read from `IndigoSecrets.py` (`LONGITUDE`) first, this field next, default -0.1246 (Big Ben) last. |
