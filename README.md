...
| 5.77.3 | 28-Aug-2026 | **The economics cluster lifted out of `plugin.py`.** Eight functions covering whole-house cost settlement and the daily, yesterday, period and calendar summaries now live in `economics.py`, which touches no Indigo API and imports nothing from `plugin` — so the import graph stays one-way and the money arithmetic can be tested without a server. `plugin.py` loses 782 lines. Verified as a faithful move rather than trusted: all eight function bodies are byte-identical to what left `plugin.py`, compared at AST level with indentation and the receiver change normalised out, and the 34 characterisation tests written beforehand pass against the extracted module. **Worth knowing about those tests:** they characterise shape and specific paths rather than arithmetic — a deliberate sign or operator flip in the gas unit cost, the no-solar comparison or the whole-house electricity total survives them. That is no risk to this release, because nothing was rewritten, but the arithmetic wants proper cover before anyone edits it. |
