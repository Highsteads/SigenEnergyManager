...
| Version | Date | Notes |
|---------|------|-------|
| 5.66.0 | 12-Aug-2026 | **A grid event that ran into dusk was reported as having had its solar shut down.** Tonight's two-hour event (18:00-20:00) went perfectly — 8.26 kWh sold, a rock-steady 4 kW out for the full two hours — but the summary that followed said the solar had been curtailed. It hadn't. The check asked what the LOWEST solar reading of the window was, and on an August evening the answer is always zero, because the sun sets. Curtailment wasn't just absent, it was impossible: the way the plugin drives an evening event, solar can only be held back once it exceeds the house load plus the 4 kW export cap — about 5 kW that evening — and it peaked at 1.76 kW. An earlier fix had already stopped this alarm firing on windows that are dark from start to finish; this one covers the window that begins in daylight and ends after sunset, which is every evening event from now until the spring. It now asks what the solar PEAKED at, since only a peak that never lifts means the panels were actually shut down, and it records that peak alongside the low reading so the figure the verdict rests on is there in black and white. Nothing about how the event is driven changed — this was only the report. Tests: 588 to 597. |
